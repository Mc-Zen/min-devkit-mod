...
public:
	...
	constexpr ValueRefOutput(const ValueRefOutput&) = delete;
	constexpr ValueRefOutput& operator=(const ValueRefOutput&) = delete;

	constexpr double getValue() const override { return value; }
	constexpr Polarity getPolarity() const override { return mode; }
	constexpr UpdateRate getUpdateRate() const override { return updateRate; }
...
template</*ModulationOperation*/ class Operation, IModulationDestination::Type mode, bool supplyCallback = true>
class ModulatableValue : public IModulationDestination
{
public:
	constexpr ModulatableValue() requires(supplyCallback) : callback([](double) {}) {}
	constexpr ModulatableValue() requires(!supplyCallback) {}
	constexpr ModulatableValue(double paramValue) : ModulatableValue() {
		setParamValue(paramValue);
	}

	template<class F>
	constexpr ModulatableValue(F&& callback, double paramValue = 0.0) requires(supplyCallback) : callback(callback) {
		setParamValue(paramValue);
	}
	...
	constexpr void setParamValue(double v) {
		...
		if constexpr (supplyCallback)
			callback(modulationValue);
	}

	constexpr void modulate(double modValue) override {
	...
	constexpr void startNewModulationCycle() override {
		modulationValue = Operation::neutral_element();
		updateModulatedValue();
	}

	template<class F>
	constexpr void setCallback(F&& callback) {
	...
	constexpr Type getType() const override { return mode; }
