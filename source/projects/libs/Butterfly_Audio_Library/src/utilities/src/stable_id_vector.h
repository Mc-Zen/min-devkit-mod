...
public:
	...
	public:
		...
		constexpr explicit redirecting_iterator() noexcept {}
		constexpr explicit redirecting_iterator(value_container& vc, index_container& ic, size_type offset = 0) noexcept
			: vc(&vc), ic(&ic), index{ offset } {}
		...
		constexpr friend redirecting_iterator operator+(const difference_type offset, const redirecting_iterator it) { return it += offset; }

		constexpr reference operator[](const difference_type offset) const noexcept { return (*vc)[(*ic)[index + offset]]; }
		constexpr std::strong_ordering operator<=>(const redirecting_iterator& a) const noexcept { return index <=> a.index; }
	...
	constexpr StableIDVector(size_t capacity) : values(capacity), value_indices(capacity) { initialize_indices(); }
	constexpr StableIDVector(size_t capacity, const T& val) : values(capacity, val), value_indices(capacity) { initialize_indices(); }
